...
export async function exportLibraryCloneForDownloadAsZipFile(libraryId: string): Promise<TransferFileMetadata> {
    ...
    try {
        ...
        await libraryLocalFolderService.exportLibraryClone(libraryId, TEMP_EXPORT_DIR)
        ...
        const zipFileName = `library_clone_${libraryId}_${new Date().toISOString()}.zip`
