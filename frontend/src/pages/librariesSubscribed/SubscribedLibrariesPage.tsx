...
export function SubscribedLibrariesPage() {
  ...
  const subscribedSpecs = specifications?.filter(
    spec => spec.distributionMethod === DistributionMethod.SUBSCRIBED
  ) || []

  const subscribedLibraries = subscribedSpecs.map(spec => {
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
      <h1 style={{ marginBottom: '20px' }}>Subscribed Libraries</h1>
      ...
      <div style={{ display: 'flex', gap: '20px' }}>
        ...
        <div style={{ 
          ...
          {subscribedLibraries.length === 0 && (
            <div style={{ padding: '20px', textAlign: 'center', color: '#666' }}>
              No subscribed libraries found
            </div>
          )}
        </div>
        ...
        <div style={{ 
          ...
          <button
            onClick={() => navigate('/libraries-subscribed/subscribe')}
            style={{ 
              padding: '8px 16px', 
          ...
          <button
            onClick={() => selectedLibraryId && navigate(`/libraries-subscribed/${selectedLibraryId}/sync`)}
